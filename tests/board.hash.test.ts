...
describe("Testing board move undo for simple pawn move", () => {
	...
	const initialBoardHash = board.hash.value;

	test("Test move and undo", () => {
		board.makeMoveFromNotation("e2e4");
		board.undoLastMove();
		expect(board.hash.value).toBe(initialBoardHash);
		expect(initialBoardHash).toBe(board.hash.getComputedHash(board));
	});
});
...
describe("Testing board move undo for promotion moves", () => {
	...
	const initialBoardHash1 = board.hash.value;
	board.makeMoveFromNotation("g7h8q");
	const initialBoardHash2 = board.hash.value;
	board.makeMoveFromNotation("b2a1q");

	test("Test undo promotions", () => {
		board.undoLastMove();
		expect(board.hash.value).toBe(initialBoardHash2);
		expect(initialBoardHash2).toBe(board.hash.getComputedHash(board));

		board.undoLastMove();
		expect(board.hash.value).toBe(initialBoardHash1);
		expect(initialBoardHash1).toBe(board.hash.getComputedHash(board));
	});
});
