...
describe("Testing board with standard start position ...", () => {
	...
	test("board has 120 elements", () => {
		expect(board._squares.length).toBe(120);
	});

	test("white to play", () => {
		expect(board._activeSide).toBe("white");
	});

	test("white king side castle is possible", () => {
		expect(board._castlingInformation.isWhiteKingSidePossible).toBe(true);
	});

	test("white queen side castle is possible", () => {
		expect(board._castlingInformation.isWhiteQueenSidePossible).toBe(true);
	});

	test("black king side castle is possible", () => {
		expect(board._castlingInformation.isBlackKingSidePossible).toBe(true);
	});

	test("black queen side castle is possible", () => {
		expect(board._castlingInformation.isBlackQueenSidePossible).toBe(true);
	});

	test("no en passant move possible", () => {
		expect(board._enPassantSquare).toBe(null);
	});

	test("half moves should be 0", () => {
		expect(board._halfMoveCountSinceLastCaptureOrPawnMove).toBe(0);
	});

	test("full moves should be 1", () => {
		expect(board._moveCount).toBe(1);
	});
});
...
describe("Testing board with middle game position ...", () => {
	...
	test("black to play", () => {
		expect(board._activeSide).toBe("black");
	});

	test("white king side castle is NOT possible", () => {
		expect(board._castlingInformation.isWhiteKingSidePossible).toBe(false);
	});

	test("white queen side castle is NOT possible", () => {
		expect(board._castlingInformation.isWhiteQueenSidePossible).toBe(false);
	});
	...
	test("black queen side castle is NOT possible", () => {
		expect(board._castlingInformation.isBlackQueenSidePossible).toBe(false);
	});
	...
	test("half moves should be 5", () => {
		expect(board._halfMoveCountSinceLastCaptureOrPawnMove).toBe(5);
	});

	test("full moves should be 8", () => {
		expect(board._moveCount).toBe(8);
	});
});
...
describe("Testing board with en passant move ...", () => {
	...
	test("en passant move on b6", () => {
		expect(board._enPassantSquare).toBe("b6");
	});

	test("square information should be set to true for en passant", () => {
		expect(board._squares[Board.getIndexFromNotation("b6")].isEnPassantSquare).toBe(true);
	});
});
